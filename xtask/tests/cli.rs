...
fn operational_failure_prints_the_diagnostic_to_stderr_and_exits_1() {
    ...
    assert!(stdout.contains("[1/17] cargo fmt --all -- --check"));
    assert!(stdout.contains("phase 1/17 failed (elapsed:"));
    assert!(stdout.contains("summary: FAILED at phase 1/17; 0 passed, 1 failed"));
    assert_eq!(
        String::from_utf8(output.stderr).expect("diagnostic is UTF-8"),
